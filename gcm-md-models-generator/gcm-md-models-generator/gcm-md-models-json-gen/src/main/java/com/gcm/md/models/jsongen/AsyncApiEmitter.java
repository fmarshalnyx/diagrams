 ...
 *   TICKS_SBE.&lt;MessageName&gt;.{symbol}  - raw SBE frames (binary)
 *   TICKS_JSON.&lt;MessageName&gt;.{symbol} - JSON frames validating against the per-message schema
 *
...
final class AsyncApiEmitter
{
    ...
    static String emit(final List<Model.Message> messages, final int schemaId, final int schemaVersion)
    {
        ...
        y.append("    Raw SBE frames are published on TICKS_SBE.* and JSON translations on TICKS_JSON.*.\n");
        ...
        for (final Model.Message m : messages)
        {
            ...
            y.append("    address: TICKS_SBE.").append(m.name).append(".{symbol}\n");
            ...
            y.append("    address: TICKS_JSON.").append(m.name).append(".{symbol}\n");
